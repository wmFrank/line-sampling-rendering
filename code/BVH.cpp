...
void BVHAccel::getLineSample(BVHBuildNode* node, float p, Intersection &pos1, Intersection &pos2, float &pdf)
{
    if(node->left == nullptr || node->right == nullptr){
        node->object->LineSample(pos1, pos2, pdf);
        return;
    }
    if(p < node->left->area) getLineSample(node->left, p, pos1, pos2, pdf);
    else getLineSample(node->right, p - node->left->area, pos1, pos2, pdf);
}

void BVHAccel::LineSample(Intersection &pos1, Intersection &pos2, float &pdf)
{
    float p = std::sqrt(get_random_float()) * root->area;
    getLineSample(root, p, pos1, pos2, pdf);
}